...
# === Output directory ===
output_dir = "/home/s3986160/master-thesis/Retention/new definition/filtered/models_community_only/modified_models/"
...
print("[Step 7] Calibrating with Platt scaling...")
calibrated_model = CalibratedClassifierCV(final_model, cv=3, method='sigmoid')
...
# === Save predictions ===
pd.DataFrame({"address": df.loc[y_test.index, "address"], "proba": test_proba, "pred": test_preds}).to_csv(
    os.path.join(output_dir, "test_predictions_calibrated.csv"), index=False
)

# === SHAP ===
explainer = shap.Explainer(final_model, X_train_res)
shap_values = explainer(X_val_imputed)

plt.figure()
shap.summary_plot(shap_values, X_val_imputed, plot_type="bar", show=False)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "shap_summary_bar_all.png"), dpi=300)
plt.show()

plt.figure()
shap.summary_plot(shap_values, X_val_imputed, show=False)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "shap_summary_dot_all.png"), dpi=300)
plt.show()

print("\u2705 Model training complete with calibrated probabilities, SHAP, and predictions saved.")
